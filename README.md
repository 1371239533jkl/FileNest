...
> **注意**：以下截图基于旧版 Catppuccin 主题，当前 UI 已基于 Ardot 设计图重构为暗蓝深空黑主题（见 [UI 设计说明](#界面设计)）。
...
- **文件搜索** — FTS5 全文搜索引擎，名称/类型/大小/日期/重复状态多条件检索，支持自然语言解读和 CSV 导出
...
### v2.1 更新亮点
- **🗄️ SQLite 嵌入式数据库** — 无需安装 MySQL，零配置开箱即用，WAL 模式高性能并发读写
- **🔎 FTS5 全文搜索** — SQLite 内置倒排索引，`MATCH` 查询比 `LIKE` 快 10-40 倍，支持前缀/短语/布尔组合
- **🎨 UI 全面重构** — 基于 Ardot 设计图，暗蓝深空黑主题 + 图标侧边导航栏 + 圆角卡片布局
...
> 支持自然语言解读（如"近一年的文档""大于100MB的图片"），由规则引擎解析。点击 **💬 AI 对话** 可跳转 AI 助手。
...
│   └── models.py             # DAO 层（8个DAO类/FTS5搜索/分页/分析/统计）
├── utils/                    # 工具函数
│   ├── __init__.py
